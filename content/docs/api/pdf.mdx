...
## Low-Level Drawing Resources

These methods create PDF resources for advanced drawing operations. See [Low-Level Drawing](/docs/advanced/low-level-drawing) for usage details.
...
page.drawRectangle({ x: 50, y: 50, width: 200, height: 100, pattern });
```

---

### createAxialShading(options)
...
### createImagePattern(options)

Create a tiling pattern from an image.

| Param            | Type       | Default  | Description           |
| ---------------- | ---------- | -------- | --------------------- |
| `options.image`  | `PDFImage` | required | Embedded image        |
| `options.width`  | `number`   | required | Tile width in points  |
| `options.height` | `number`   | required | Tile height in points |
...
page.drawCircle({ x: 200, y: 400, radius: 80, pattern });
```

---

### createShadingPattern(options)
